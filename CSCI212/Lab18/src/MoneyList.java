...
public class MoneyList {
	...
	/**
	 * Returns the value of the contents of this money list.
	 * @return Value of the contents as a float value.
	 */
	public float getValue() {
		float sum = 0F;
		MoneyNode p = first.next;
		while (p != null) {
			if (p.data instanceof Coin) //if data in node is a Coin type
				sum += p.data.getValue() / 100F; 
			else //otherwise data in node is a Bill type
				sum += p.data.getValue();
